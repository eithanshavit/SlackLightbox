...
function handleSearchResults(results) {
  ...
  var item = images[0];
  transitionToImage(item);
}

// Stretch-fit image
function fitImage(imageObj) {
  var viewport = document.getElementById('viewport-top');
  ...
  var srcW = imageObj.width;
  var srcH = imageObj.height;
  var srcAspect = srcW/srcH;

  if (srcAspect > destAspect) {
    // Src is wider
    viewport.style.width = 'auto';
    viewport.style.height = destH;
    viewport.style.top = 0;
    viewport.style.left = (destW - destH * srcAspect) / 2;
  }
  else {
    // Src is taller
    viewport.style.width = destW;
    viewport.style.height = 'auto';
    viewport.style.top = (destH - destW / srcAspect) / 2;
...
function transitionToImage(imageItem) {
  var img = new Image();
  img.onload = function() {
    fitImage(imageItem.image);
    document.getElementById("viewport-top").src = imageItem.link;
  }
  img.src = imageItem.link;
}
